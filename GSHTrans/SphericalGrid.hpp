...
#pragma omp parallel for schedule(static) \
 ...
 private:
  ...
  static int ThreadCount(Execution policy) {
    return policy.Threads() > 0 ? policy.Threads() : OpenMP::MaxThreads();
  }
...
#pragma omp parallel num_threads(threadCount)
      {
        const auto thread = static_cast<Int>(OpenMP::ThreadNumber());
        const auto threads = static_cast<Int>(OpenMP::TeamThreads());
        using Work =
