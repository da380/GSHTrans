...
template <std::ptrdiff_t _N, typename _Grid, RealOrComplexValued _Value>
struct Traits<CanonicalComponentField<_N, _Grid, _Value>> {
...
class CanonicalComponentField
    : public CanonicalComponentFieldBase<
          _N, CanonicalComponentField<_N, _Grid, _Value>> {
 public:
  using Value = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Value;
  using Int = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Int;
  using Real = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Real;
  using Complex = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Complex;
  using Scalar = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Scalar;
  using Writeable = typename Internal::Traits<
      CanonicalComponentField<_N, _Grid, _Value>>::Writeable;
  ...
  template <typename Derived>
  requires std::convertible_to<typename Derived::Scalar, Scalar>
  CanonicalComponentField(CanonicalComponentFieldBase<_N, Derived>&& other)
...
using RealCanonicalComponentField =
    CanonicalComponentField<N, Grid, RealValued>;

template <std::ptrdiff_t N, typename Grid>
requires std::derived_from<Grid, GridBase<Grid>>
using ComplexCanonicalComponentField =
    CanonicalComponentField<N, Grid, ComplexValued>;

// Type aliases for scalar fields.
template <typename Grid, RealOrComplexValued Value>
requires std::derived_from<Grid, GridBase<Grid>>
using ScalarField = CanonicalComponentField<0, Grid, Value>;

template <typename Grid>
requires std::derived_from<Grid, GridBase<Grid>>
using RealScalarField = CanonicalComponentField<0, Grid, RealValued>;

template <typename Grid>
requires std::derived_from<Grid, GridBase<Grid>>
using ComplexScalarField = CanonicalComponentField<0, Grid, ComplexValued>;
