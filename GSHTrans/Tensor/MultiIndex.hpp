...
/// against it stays written once. Orbits.hpp walks the group it is given over
/// the components it is told about, the symmetry policies permute slots by
/// position and never look at their contents, and storage groups by the slot
/// sum -- none of which knows or needs to know how many values a slot can take.
///
