...
\begin{quote}
  ...
  gradient of the tensor.}  It gives the $\eth$ of each component as a
...
$\partial^{\sigma}$ is not currently implemented.  Implementing it is
well-defined and fits the existing machinery: the slot shifts couple
different components \emph{at the same $(l,m)$}, which is exactly what a
tensor expansion holds, and prepending a slot is what \code{MultiIndex}
already does.  Two things would need deciding.  The $\sigma=0$ component is
the radial derivative, which an angular library cannot supply --- though
the \emph{surface} gradient $\nabla_1$ has no $\ebasis{0}$ component at all
(D\&T C.145), so a surface-gradient operator is complete without it.  And
on a reality-reduced tensor a shifted slot may name a derived component,
whose spectral form reverses the order index by \eqref{eq:complevel} rather
than acting at fixed $(l,m)$.
...
(\partial^\sigma T^{\alpha\ldots})P^{\beta\ldots} +
T^{\alpha\ldots}(\partial^\sigma P^{\beta\ldots})$.  It does \emph{not}
...
above about sixteen threads, since a per-$m$ decomposition has disjoint
outputs and needs neither accumulator nor reduction.  It is the natural
next piece of performance work.
