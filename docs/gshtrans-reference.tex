...
Two statements in this document cannot be settled from inside the library.

\begin{enumerate}
  \item The sign $\mp$ in \eqref{eq:basis}.  Some authors use
    $\ebasis{\pm}=\pm(\hat{\boldsymbol{\theta}}\pm
    i\hat{\boldsymbol{\phi}})/\sqrt{2}$, which flips every odd-rank
    reality phase.  Nothing the library currently computes observes it.
  \item The overall signs of $\eth$ and $\conj{\eth}$
    (\S\ref{sec:eth}).  The magnitudes are not in doubt, and two
    identities pin everything else, but both survive flipping the pair.
\end{enumerate}
...
\paragraph{Two identities, and what they do not pin.}  Composing the
factors,
\begin{equation}
...
second is structural.  Both are tested.  \textbf{Both survive flipping the
sign of the pair}, so the overall sign in \eqref{eq:eth} remains a
convention to be fixed against Phinney \& Burridge, and is implemented as
stated above.
...
reach for first.  They are simply not the tensor operator, and nothing in
their types says so, because a tensor expansion's components are ordinary
spin expansions.
...
This is the one substantial piece of performance work the library has not
done, and \code{core-plan.md} \S10 names it as the lever aimed at what
actually binds.  What follows is how it would work and why it should help,
...
The transpose itself is free, for the same reason it was free in tier 1
(P6).  A single \code{plan\_many} over $n_\theta k$ rows with output stride
$n_\theta k$ and output distance 1 writes the result directly in
$[m][\theta][\kappa]$ order.
...
\paragraph{It fights on-the-fly generation.}  The recursion runs up degrees
at fixed $(n,\theta)$, which produces values in exactly the order the
\emph{present} layout stores them and exactly the wrong order for
$[n][m][l][\theta]$.  A generating supplier feeding a GEMM would have to
transpose, which is the traffic it exists to avoid.  Since the generated
path measured slower anyway (\S\ref{sec:storage}), that is not a conflict
to resolve so much as a reason the two were never going to be combined.

\paragraph{It costs the exact-equality tests.}  A GEMM sums in whatever
order its kernel chooses, so a GEMM path cannot be bit-compared against the
current one; the batched-against-unbatched tests, which presently demand
exact equality precisely because tier 1 does \emph{not} reorder any sum,
would have to become tolerance comparisons for that path.  That is a real
loss of test strength and should be a deliberate trade rather than a
discovery.

\paragraph{Which BLAS.}  Eigen is already a dependency and has a competent
GEMM; an external OpenBLAS or MKL would likely beat it on this shape, at
the cost of a hard dependency.  Starting with Eigen and measuring against
a linked BLAS is the cheap order.
...
\paragraph{A transform-major layout with a GEMM} is the one restructure
aimed at what actually binds --- the per-element load/store count --- and
it subsumes the question of how the forward transform should decompose
above about sixteen threads.  It is the natural next piece of performance
work, and \S\ref{sec:gemm} sets it out in full.

\paragraph{Polar truncation}, skipping the $(m,\theta)$ pairs where
$d^l_{Nm}$ is negligible --- it falls off like $\sin^{|m|}\theta$ --- is
the one option that makes the problem smaller rather than making the
machine work harder, and it reduces storage and arithmetic together.  It
is not implemented and probably dominates both of the reductions above.

\paragraph{The two open conventions} of \S\ref{sec:conventions} want an
answer from the literature rather than from a test.
