...
\begin{equation}
  Y^{N}_{lm}(\theta,\phi)
    = \sqrt{\frac{2l+1}{4\pi}}\; d^{l}_{mN}(\theta)\, e^{im\phi}
  \label{eq:gsh}
\end{equation}
in the \texttt{Ortho} normalisation, where $d^{l}_{mN}$ is the real
Wigner $d$-function.  These are orthonormal on the sphere,
$\int \conj{Y^{N}_{lm}} Y^{N}_{l'm'} \dOmega = \delta_{ll'}\delta_{mm'}$.
The sum over $l$ starts at $|N|$ because $d^{l}_{mN}$ vanishes
identically for $l < |N|$.

\paragraph{Convention check.}  The index order in $d^{l}_{mN}$, and
whether the code's \texttt{Wigner} class stores $d^{l}_{mn}$ or
$d^{l}_{nm}$, should be confirmed against \texttt{src/Wigner.h}.  The
normalisation in \eqref{eq:gsh} is however pinned by the $l=0$ behaviour
...
Using $d^{l}_{-m,-N} = (-1)^{m-N} d^{l}_{mN}$ and the reality of
$d^{l}_{mN}$, equation \eqref{eq:gsh} gives
\begin{equation}
